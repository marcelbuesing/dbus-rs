...
impl AConnection {
    ...
    pub fn new(c: Arc<Mutex<Connection>>) -> Box<Future<Item = AConnection, Error = &'static str> + Send>  {
      ...
      let mut d = ADriver {
        ...
        {
            let c_locked = c.lock().unwrap();
            c_locked.set_watch_callback(Box::new(|_| unimplemented!("Watch handling is very rare and not implemented yet")));
        }
        ...
        for w in i.conn.lock().unwrap().watch_fds() {
            if let Err(e) = d.modify_watch(w, false) {
                return Box::new(future::err::<AConnection, _>("Failed to modify watch"));
            }
        }

        let f = d.and_then(|_| future::ok(i)).map_err(|_| "");
        Box::new(f)
    }
    ...
    pub fn method_call(&self, m: Message) -> Result<AMethodCall, &'static str> {
        let r = self.conn.lock().unwrap().send(m).map_err(|_| "D-Bus send error")?;
        let (tx, rx) = oneshot::channel();
        let mut map = self.callmap.lock().map_err(|_| "Callmap poisoned")?;
...
impl Drop for AConnection {
    fn drop(&mut self) {
        ...
        self.conn.lock().unwrap().set_watch_callback(Box::new(|_| {}));
    }
}
...
impl ADriver {
    ...
    fn send_stream(&self, m: Message) {
        self.msgstream.lock().unwrap().as_ref().map(|z| { z.unbounded_send(m).unwrap() });
    }

    fn handle_msgs(&mut self) {
        let conn = self.conn.lock().unwrap();
        let msgs = ConnMsgs { conn: &*conn, timeout_ms: None };
        for m in msgs {
            debug!("handle_msgs: {:?}", m);
...
impl Future for ADriver {
    ...
    fn poll(&mut self) -> Result<Async<()>, ()> {
        ...
        for w in self.fds.values() {
            ...
            self.conn.lock().unwrap().watch_handle(w.get_ref().0.fd(), flags);
